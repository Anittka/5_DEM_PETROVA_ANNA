...
class ProductsWindow(ctk.CTk):
    ...
    def create_search_filters(self):
        ...
        self.search_frame = ctk.CTkFrame(self, fg_color="#F5DEB3", corner_radius=10)
        self.search_frame.pack(fill="x", padx=10, pady=5)
        
        # Поиск
        ctk.CTkLabel(self.search_frame, text="Поиск:", font=("Arial", 14)).pack(side="left", padx=5)
        self.search_entry = ctk.CTkEntry(self.search_frame, width=300, font=("Arial", 14), placeholder_text="Название, категория, описание...")
        self.search_entry.pack(side="left", padx=5)
        self.search_entry.bind('<KeyRelease>', lambda e: self.load_products())
        
        # Фильтр по поставщику (выпадающий список)
        ctk.CTkLabel(self.search_frame, text="Поставщик:", font=("Arial", 14)).pack(side="left", padx=50)
        ...
        # Сортировка
        ctk.CTkLabel(self.search_frame, text="Сортировка:", font=("Arial", 14)).pack(side="left", padx=50)
    ...
    def create_buttons(self):
        ...
        self.button_frame.pack(fill="x", padx=30, pady=30)
        
        if self.role == 'admin':
            ctk.CTkButton(self.button_frame, text="Добавить товар", command=self.add_product, fg_color="#DEB887").pack(side="left", padx=20)
            ctk.CTkButton(self.button_frame, text="Импорт товаров", command=self.import_products, fg_color="#DEB887").pack(side="left", padx=20)
            ctk.CTkButton(self.button_frame, text="Импорт пользователей", command=self.import_users, fg_color="#DEB887").pack(side="left", padx=20)
            ctk.CTkButton(self.button_frame, text="Импорт заказов", command=self.import_orders, fg_color="#DEB887").pack(side="left", padx=20)
            ctk.CTkButton(self.button_frame, text="Заказы", command=self.open_orders, fg_color="#DEB887").pack(side="left", padx=5)
        
        elif self.role == 'manager':
            ctk.CTkButton(self.button_frame, text="Заказы", command=self.open_orders, fg_color="#DEB887").pack(side="left", padx=20)
        
        # Кнопка "Назад" для всех
        ctk.CTkButton(self.button_frame, text="Выход", command=self.logout, fg_color="#DEB887", text_color="#000000").pack(side="right", padx=20)
    ...
    def load_products(self):
        ...
        for idx, row in enumerate(rows):
            ...
            row_num = idx // 2
            col_num = idx % 2
            card.grid(row=row_num, column=col_num, padx=30, pady=20, sticky="nsew")
    ...
    def create_product_card(self, product_id, name, category, description, manufacturer, supplier_name, price, unit, quantity, discount, image_path):
        ...
        left_frame = ctk.CTkFrame(card, fg_color="transparent", width=220, height=260)
