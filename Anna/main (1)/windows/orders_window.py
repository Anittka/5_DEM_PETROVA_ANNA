...
class OrdersWindow(ctk.CTkToplevel):
    def __init__(self, parent, role):
        ...
        if role == 'admin':
            ...
            self.button_frame.pack(fill="x", padx=60, pady=60)
    ...
    def open_order_form(self, order_id=None):
        ...
        # Артикул
        ctk.CTkLabel(form, text="Артикул:", font=("Arial", 14)).pack(anchor="w", padx=50, pady=20)
        article_entry = ctk.CTkEntry(form, width=600, font=("Arial", 14))
        article_entry.pack(padx=400, pady=20)
        
        # Статус
        ctk.CTkLabel(form, text="Статус:", font=("Arial", 14)).pack(anchor="w", padx=50, pady=20)
        status_var = ctk.StringVar(value="Новый")
        status_combo = ctk.CTkComboBox(
            form, values=["Новый", "В обработке", "Выдан"],
            variable=status_var, width=600, font=("Arial", 14)
        )
        status_combo.pack(padx=50, pady=20)
        
        # Адрес выдачи
        ctk.CTkLabel(form, text="Адрес пункта выдачи:", font=("Arial", 14)).pack(anchor="w", padx=50, pady=20)
        address_entry = ctk.CTkEntry(form, width=600, font=("Arial", 14))
        address_entry.pack(padx=50, pady=20)
        
        # Дата заказа
        ctk.CTkLabel(form, text="Дата заказа (ГГГГ-ММ-ДД):", font=("Arial", 14)).pack(anchor="w", padx=50, pady=20)
        order_date_entry = ctk.CTkEntry(form, width=600, font=("Arial", 14), placeholder_text=datetime.now().strftime("%Y-%m-%d"))
        order_date_entry.pack(padx=50, pady=20)
        
        # Дата выдачи
        ctk.CTkLabel(form, text="Дата выдачи (ГГГГ-ММ-ДД):", font=("Arial", 14)).pack(anchor="w", padx=50, pady=20)
        issue_date_entry = ctk.CTkEntry(form, width=600, font=("Arial", 14))
        issue_date_entry.pack(padx=50, pady=20)
        ...
        ctk.CTkButton(btn_frame, text="Сохранить", command=save, fg_color="#DEB887", font=("Comic Sans MS", 14), width=120).pack(side="left", padx=10)
        ctk.CTkButton(btn_frame, text="Отмена", command=form.destroy, fg_color="#DEB887", text_color="#000000", font=("Comic Sans MS", 14), width=120).pack(side="left", padx=10)